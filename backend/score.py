...
from src.mekg.api import router as mekg_router
from src.mekg.mcp_server import mcp_app, mcp_lifespan
from src.mekg.api import get_service as get_mekg_service
...
app.include_router(mekg_router)
...
async def extract_knowledge_graph_from_file(
    ...
    try:
        ...
        if params.source_type == 'local file' and params.extraction_profile == 'mekg':
            mekg_result = await get_mekg_service().ingest_path(
                merged_file_path,
                source_locator=params.file_name,
                enable_vision=True,
                enable_extraction=True,
            )
            graph.query(
                """
                MATCH (d:Document {fileName:$file_name}) WHERE NOT d:MEKG
                SET d.status='Completed', d.model=$model, d.nodeCount=$node_count,
                    d.relationshipCount=$relationship_count, d.updatedAt=datetime()
                """,
                {
                    "file_name": params.file_name,
                    "model": params.model,
                    "node_count": sum(mekg_result.get("extracted", {}).values()),
            ...
            return create_api_response(
                ...
                file_source=params.source_type,
            )
