...
def get_llm(model: str):
    ...
    is_yandex = "YANDEX" in model
    env_key = "LLM_MODEL" if is_yandex else f"LLM_MODEL_CONFIG_{model}"
    ...
    try:
        if is_yandex:
            ...
            api_key = get_value_from_env("YANDEX_API_KEY")
            folder_id = get_value_from_env("YANDEX_FOLDER_ID")
            api_endpoint = get_value_from_env(
            ...
            object.__setattr__(llm, "_graph_builder_provider", "yandex")
...
def _should_retry_yandex_without_structured_output(llm, error: Exception) -> bool:
    """Retry only Yandex 4xx capability errors in prompt-based JSON mode."""
    if getattr(llm, "_graph_builder_provider", None) != "yandex":
        return False
    ...
    try:
        ...
        if isinstance(llm,DiffbotGraphTransformer):
            ...
            try:
                ...
                if ignore_tool_usage or not _should_retry_yandex_without_structured_output(llm, structured_error):
                    raise
                logging.warning(
                    "Yandex structured output was rejected; retrying once with prompt-based JSON extraction"
                )
                fallback_transformer = LLMGraphTransformer(
                    llm=llm,
                    node_properties=False,
                    relationship_properties=False,
                    allowed_nodes=allowedNodes,
                ...
                graph_document_list = await fallback_transformer.aconvert_to_graph_documents(
                    combined_chunk_document_list
                )
    except Exception as e:
