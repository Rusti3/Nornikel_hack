...
def create_chunk_embeddings(graph, chunkId_chunkDoc_list, file_name, embedding_provider, embedding_model):
    ...
    for row in chunkId_chunkDoc_list:
        if isEmbedding:
            embeddings_arr = embeddings.embed_documents([row['chunk_doc'].page_content])[0]
...
def create_chunk_vector_index(graph, embedding_provider, embedding_model):
    ...
    try:
        embeddings, embedding_dimension = load_embedding_model(embedding_provider, embedding_model)
        vector_index_query = """
            SHOW INDEXES YIELD name, type, labelsOrTypes, properties, options
            WHERE name = 'vector'
              AND type = 'VECTOR'
              AND 'Chunk' IN labelsOrTypes
        ...
        existing_dimension = int(vector_index[0]['dimensions']) if vector_index else None

        if existing_dimension is not None and existing_dimension != embedding_dimension:
            logging.warning(
                "Vector index dimension mismatch (%s != %s); recreating index",
                existing_dimension,
                embedding_dimension,
            )
            execute_graph_query(graph, "DROP INDEX vector IF EXISTS")
            vector_index = []

        if not vector_index:
            vector_store = Neo4jVector(embedding=embeddings,
                                    graph=graph,
                                    node_label="Chunk", 
