...
Важно: готовые seed-дампы — это демо-срез, а не полностью обработанный
исходный пакет документов. Часть документов уже доступна в поиске/чанках и
графе, но не каждый файл успел получить полный набор embeddings и извлечённых
Neo4j-фактов к дедлайну. Оставшиеся или новые документы можно дозагрузить через
вкладку **Данные**: они пройдут тот же pipeline индексации и добавятся в общий
поиск/граф.
