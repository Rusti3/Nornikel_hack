...
export default class ErrorBoundary extends React.Component<any, any> {
  ...
  render() {
    if (this.state.hasError) {
      const showDebugError = import.meta.env.DEV || window.location.hostname === 'localhost';
      ...
      return (
        <div className='n-size-full n-flex n-flex-col n-items-center n-justify-center n-rounded-md n-bg-palette-neutral-bg-weak n-box-border'>
          <Banner
            ...
            type='info'
            description={
              showDebugError && this.state.errorMessage
                ? `${fallbackDescription}: ${this.state.errorName} ${this.state.errorMessage}`
                : fallbackDescription
            }
