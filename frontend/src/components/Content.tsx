...
import MEKGPanel from './MEKG/MEKGPanel';
...
const Content: React.FC<ContentProps> = ({
  ...
  const [showMEKGPanel, setShowMEKGPanel] = useState<boolean>(false);
    ...
    setMessages([
      {
        ...
        modes: {
          agentic_rag: {
            message:
              'Добро пожаловать в исследовательский чат MEKG. Задайте технический вопрос — агент соберёт evidence из Postgres/Neo4j, покажет источники, пробелы, противоречия и уверенность ответа.',
          },
        },
        user: 'chatbot',
        currentMode: 'agentic_rag',
      },
    ]);
    setchatModes([chatModeLables['agentic rag']]);
  };
  ...
  return (
    <>
      ...
      <MEKGPanel open={showMEKGPanel} onClose={() => setShowMEKGPanel(false)} />
      <GraphViewModal
      ...
      <div className={`n-bg-palette-neutral-bg-default main-content-wrapper`}>
        ...
        >
          ...
          <div className='enhancement-btn__wrapper'>
            <ButtonWithToolTip
              placement='top'
              text='MEKG profile, evidence validation and quality audit'
              label='MEKG Evidence Profile'
              className='mr-2!'
              onClick={() => setShowMEKGPanel(true)}
